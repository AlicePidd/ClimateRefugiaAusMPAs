...
# Folders ----------------------------------------------------------------------
  ...
  density_fol <- make_folder(source_disk, "MHW", var_nm[1], "density1") 
  densityplot_fol <- make_folder(source_disk, "MHW", var_nm[1], "density1/plots") 
  threatlayer_fol <- make_folder(source_disk, "MHW", var_nm[1], "threat_layers1") 
  
  
  
# Data -------------------------------------------------------------------------

  combined_ssp_dfs <- readRDS(paste0(density_fol, "/", var_nm[1], "_combinedSSP_mid-term_dflist.RDS"))
  combined_ssp_dfs
...
# Density plot -----------------------------------------------------------------
  ...
  plot_density <- function(d) {
    ...
    imap(d, function(df, ssp) {
      ...
      ggsave(paste0(densityplot_fol, "/density_plot_median_", term, "_orange_", ssp, "_", var_nm[1], ".pdf"),
             p,
             width = 8,
             height = 6,
             dpi = 300)
